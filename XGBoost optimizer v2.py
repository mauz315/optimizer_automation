...
from model_dump import hyp_dump
...
sheet_name = "PythonMoY"
model_name = "pbiperu_3lagMoMv2.sav"
model_desc = "Modelo solo con 3 rezagos. Variaciones YoY y variaciones MoM, resultado en MoM"  # Pequeña descripción del modelo
...
dataset = create_target(dataset, 'PRGDP Index MoM', 1)
# dataset = create_target(dataset, 'PRGOGGDP Index', 1)
# Target series:
# PRGDP Index: nsa
# PRGOGGDP Index: sa
del dataset['PRGDP Index MoM']
del dataset['PRGDP Index YoY']
del dataset['PRGOGGDP Index MoM']
del dataset['PRGOGGDP Index YoY']
dataset = select_best_lags(dataset, dataset.iloc[:int(len(dataset) * percent_train)], 3, replicate=True, n_replicas=3)
# Con esta función se seleccionan los lags con mayor poder predictivo sobre la variable target.
...
# En caso de haber usado mse como métrica de evaluación y no querer usar validación cruzada,
# se define la variable piter que almacena la parte del índice del df donde se deberá testear el modelo.
# Además, en la función objective_function, se debe asgignar, en la variable reg, piter a cv.
...
search_space = hp.choice('regressor',[
    {
        ...
        'preprocessing': hp.choice('preprocessing', ['NoTransform', 'MinMaxScaler',
                         ...
                         'PowerTransformer']),
        'k_features': hp.quniform('k_features', 3, 5, 1),
...
def select_model(space):
    ...
    return hyperopt.space_eval(space, best_regressor)


best_model = select_model(search_space)
model_metadata = {
    "desc": model_desc,
    "obs": len(dataset),
    "train_size": length_train,
    "test_size": length_train - length_opt
}

hyp_dump(best_model, model_metadata, model_name)
# print("Chosen variables are {}".format(best_features))
